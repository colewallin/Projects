...
(*All the work above this comment was done by Eric Van Wyke. Everything Below
it is done by Cole Wallin*)

let rec str_from (n:int) =
  Cons ( string_of_int n,
         fun () -> str_from (n+1) )

let str_nats = str_from 1

let rec helper n x sep = 
  if x = 0 then Cons("\n", fun () -> helper n n sep)
  else Cons(sep, fun () -> helper n (x-1) sep)

let rec separators n sep= helper n n sep

let ten_per_line = separators 9 (", ")

let rec alternate s1 s2 =
  match s1, s2 with
  |Cons(a, b), Cons(c, d) -> Cons(a, fun () -> Cons(c, fun () -> alternate (b ()) (d ())))

let str_105_nats = List.fold_left (^) "" (take 209 (alternate (str_nats) (ten_per_line)))
