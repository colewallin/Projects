...
let program_4 =
  ...
  Seq(IfThen(Eq(Mod(Var "x", Value(Int 2)), Value(Int 0)),
             Assign("y", Add(Var "y", Value(Int 2)))),
  Seq(IfThen(Eq(Mod(Var "x", Value(Int 3)), Value(Int 0)),
...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | IfThenElse (cond, condthen, condelse) -> (match eval cond stt with
                                             |Bool(true) -> exec condthen stt
                                             |Bool(false) -> exec condelse stt
                                             )
  | While (cond, body) ->
  ...
  | IfThen (cond, body) -> exec (IfThenElse(cond, body, Skip)) stt
  | Skip -> stt
